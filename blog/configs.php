...
if ($installed!='yes') {	
	$connDB = mysqli_connect($CONFIG["hostname"], $CONFIG["mysql_user"], $CONFIG["mysql_password"], $CONFIG["mysql_database"]);
...
if(!function_exists('sql_result')){ 
	function sql_result($sql) {
		global $connDB;
		$sql_result = mysqli_query ($connDB, $sql) or die ('Could not execute MySQL query: '.$sql.' . Error: '.mysqli_error($connDB));
		return $sql_result;
	}
}
...
if(!function_exists('SafetyDB')){ 
	function SafetyDB($str) {
		global $connDB;
		return mysqli_real_escape_string($connDB, $str); 
	}
}
 ...
 */
if(!function_exists('truncateBlogHtml')){ 
...
if (!function_exists("url_slug")) { 
	function url_slug($str, $options = array()) {
		...
		$char_map = array(
			...
			'Ч' => 'Ch', 'Ш' => 'Sh', 'Щ' => 'Sh', 'Ъ' => 'Y', 'Ы' => 'Y', 'Ь' => '', 'Э' => 'E', 'Ю' => 'Yu',
			'Я' => 'Ya',
			'а' => 'a', 'б' => 'b', 'в' => 'v', 'г' => 'g', 'д' => 'd', 'е' => 'e', 'ё' => 'yo', 'ж' => 'zh',
			'з' => 'z', 'и' => 'i', 'й' => 'j', 'к' => 'k', 'л' => 'l', 'м' => 'm', 'н' => 'n', 'о' => 'o',
			'п' => 'p', 'р' => 'r', 'с' => 's', 'т' => 't', 'у' => 'u', 'ф' => 'f', 'х' => 'h', 'ц' => 'c',
...
require_once(dirname(__FILE__).'/include/classes.php');

if (!defined('RECAPTCHA_API_SERVER')) {
    require_once('recaptchalib.php');
}

$configs_are_set_designblog = 1;
?>